...
# to add the appropriate lines to your git pre-commit hook file use the following command:
#
#    ./version_bump.sh precommit
#

VERSION_GO_FILEPATH="internal/util/version.go"
...
bump_build() {
  if ! CURRENT_BUILD_NUM=$(grep -o "internalBuildNumber\\s*\(int64\|int\)\?\\s*=\\s*[0-9]\\+" $VERSION_GO_FILEPATH | grep -o " = [0-9]\\+" | grep -o "[0-9]\\+"); then
  ...
  sed -i.bak "s/internalBuildNumber[[:space:]]*[int64]*[[:space:]]*=[[:space:]]*[0-9]*/internalBuildNumber\\ int64\\ =\\ ${NEW_NUM}/g" $VERSION_GO_FILEPATH

  NEW_TS=$(date +%s)
  echo "Bumping build timestamp new=$NEW_TS"

  sed -i.bak "s/internalBuildTimestamp[[:space:]]*[int64]*[[:space:]]*=[[:space:]]*[0-9][0-9]*/internalBuildTimestamp\\ int64\\ =\\ ${NEW_TS}/g" $VERSION_GO_FILEPATH

  rm -f ${VERSION_GO_FILEPATH}.bak

  # cleanup
  go fmt $VERSION_GO_FILEPATH >/dev/null

  FINAL_VERSION_STRING=$(grep -o "internalVersionString\\s*=\\s\"[vV]*[0-9]*\.[0-9]*\.[0-9]*\"\\+" $VERSION_GO_FILEPATH | grep -o "[0-9]*\.[0-9]*\.[0-9]*")
  echo "v${FINAL_VERSION_STRING}"
}

bump_version_string() {
  echo "Bumping ${POSITION} number, from ${XYZ_VERSION_STRING} to ${NEW_VERSION_STRING}"
  sed -i.bak "s/internalVersionString[[:space:]]*=[[:space:]]*\"[vV]*[0-9]*\.[0-9]*\.[0-9]*\"/internalVersionString\\ =\\ \"${NEW_VERSION_STRING}\"/g" $VERSION_GO_FILEPATH
...
precommit() {
  #TODO need to check if these lines already exist in the file
  {
    ...
    echo './version_bump.sh'
    echo "git add $VERSION_GO_FILEPATH"
...
while [ "$1" != "" ]; do
  ...
  XYZ_VERSION_STRING=$(grep -o "internalVersionString\\s*=\\s\"[vV]*[0-9]*\.[0-9]*\.[0-9]*\"\\+" "$VERSION_GO_FILEPATH" | grep -o "[0-9]*\.[0-9]*\.[0-9]*")
